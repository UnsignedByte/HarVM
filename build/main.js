(function (crypto, stream, zlib, net, querystring, constants, dns) {
  'use strict';
...
> ${CODE}`;
  ...
  */
  ...
  ({ host, port } = get);
  	...
  	if (!mcproto) mcproto = (await Promise.resolve().then(function () { return index$1; })).default;
  ...
  fetch$1.auth = ['manage guild'];
  fetch$1.parser = new BashlikeArgumentParser([
  	{
  		name: 'help',
  		aliases: ['h', '?'],
  ...
  class DataManager {
  	...
  	async init(){
  		if (isNode()) {
  			...
  			await Promise.resolve().then(function () { return localforage$2; });
  ...
  */
  ...
  var snbt = createCommonjsModule(function (module, exports) {
  Object.defineProperty(exports, "__esModule", { value: true });
  ...
  function stringify(tag, options = {}) {
      ...
      function stringify(tag, depth) {
          ...
          if (tag instanceof lib.Byte)
              return `${tag.value}b`;
          else if (tag instanceof lib.Short)
              return `${tag.value}s`;
          else if (tag instanceof lib.Int)
              return `${tag.value | 0}`;
          else if (typeof tag == "bigint")
              return `${tag}l`;
          else if (tag instanceof lib.Float)
              return `${tag.value}f`;
      ...
      function readNumber() {
          ...
          while (index < text.length) {
              ...
              if ("0123456789".includes(char)) {
                  ...
                  return new lib.Float(+text.slice(i, index - 1));
              }
              else if (char == "d" || char == "D") {
                  return +text.slice(i, index - 1);
              }
              else if (char == "b" || char == "B") {
                  return new lib.Byte(+text.slice(i, index - 1));
              }
              else if (char == "s" || char == "S") {
                  return new lib.Short(+text.slice(i, index - 1));
              }
                  ...
                  return new lib.Int(+text.slice(i, --index));
          }
          if (hasFloatingPoint)
              return +text.slice(i, index);
          else
   ...
   */
  function encode(name, tag$1) {
      let buffer = Buffer.alloc(1024), offset = 0;
      // write tag type
      offset = buffer.writeUInt8(tag$1 == null ? tag.TagType.End : tag.getTagType(tag$1), offset);
      // write tag name
      if (tag$1 != null && name != null)
          ({ buffer, offset } = writeString(name, buffer, offset));
      // write tag value
      if (tag$1 != null)
          ({ buffer, offset } = encodeTagValue(tag$1, buffer, offset));
  ...
  function decodeTagValue(type, buffer, offset, useMaps) {
      let value;
      switch (type) {
          case tag.TagType.Byte:
              value = new tag.Byte(buffer.readInt8((offset += 1) - 1));
              break;
          case tag.TagType.Short:
              value = new tag.Short(buffer.readInt16BE((offset += 2) - 2));
              break;
          case tag.TagType.Int:
              value = new tag.Int(buffer.readInt32BE((offset += 4) - 4));
              break;
          case tag.TagType.Long:
              {
                  value = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength).getBigInt64(offset);
                  offset += 8;
              }
              break;
          case tag.TagType.Float:
              value = new tag.Float(buffer.readFloatBE((offset += 4) - 4));
              break;
          case tag.TagType.Double:
              value = buffer.readDoubleBE((offset += 8) - 8);
              break;
          case tag.TagType.ByteArray: {
  ...
  function encodeTagValue(tag$1, buffer, offset) {
      // since most of the data types are smaller than 8 bytes, allocate this amount
      buffer = accommodate(buffer, offset, 8);
      if (tag$1 instanceof tag.Byte) {
          offset = tag$1.value < 0
              ? buffer.writeInt8(tag$1.value, offset)
              : buffer.writeUInt8(tag$1.value, offset);
      }
      else if (tag$1 instanceof tag.Short) {
          offset = tag$1.value < 0
              ? buffer.writeInt16BE(tag$1.value, offset, false)
              : buffer.writeUInt16BE(tag$1.value, offset, false);
      }
      else if (tag$1 instanceof tag.Int) {
          offset = tag$1.value < 0
              ? buffer.writeInt32BE(tag$1.value, offset)
              : buffer.writeUInt32BE(tag$1.value, offset);
      }
      else if (typeof tag$1 == "bigint") {
          new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength).setBigInt64(offset, tag$1);
          offset += 8;
      }
      else if (tag$1 instanceof tag.Float) {
          offset = buffer.writeFloatBE(tag$1.value, offset);
      }
      else if (typeof tag$1 == "number") {
          offset = buffer.writeDoubleBE(tag$1, offset);
      }
      else if (tag$1 instanceof Buffer || tag$1 instanceof Int8Array) {
          offset = buffer.writeUInt32BE(tag$1.length, offset);
          buffer = accommodate(buffer, offset, tag$1.length);
          (tag$1 instanceof Buffer ? tag$1 : Buffer.from(tag$1)).copy(buffer, offset);
          offset += tag$1.length;
      }
      else if (tag$1 instanceof Array) {
          const type = tag$1.length > 0 ? tag.getTagType(tag$1[0]) : tag.TagType.End;
          offset = buffer.writeUInt8(type, offset);
          offset = buffer.writeUInt32BE(tag$1.length, offset);
          for (const item of tag$1) {
              if (tag.getTagType(item) != type)
                  throw new Error("Odd tag type in list");
              ({ buffer, offset } = encodeTagValue(item, buffer, offset));
          }
      }
      else if (typeof tag$1 == "string") {
          ({ buffer, offset } = writeString(tag$1, buffer, offset));
      }
      else if (tag$1 instanceof Int32Array) {
          offset = buffer.writeUInt32BE(tag$1.length, offset);
          buffer = accommodate(buffer, offset, tag$1.byteLength);
          for (let i = 0; i < tag$1.length; i++) {
              buffer.writeInt32BE(tag$1[i], offset + i * 4);
          }
          offset += tag$1.byteLength;
      }
      else if (tag$1 instanceof BigInt64Array) {
          offset = buffer.writeUInt32BE(tag$1.length, offset);
          buffer = accommodate(buffer, offset, tag$1.byteLength);
          const dataview = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
          for (let i = 0; i < tag$1.length; i++) {
              dataview.setBigInt64(offset + i * 8, tag$1[i]);
          }
          offset += tag$1.byteLength;
      }
      else {
          for (const [key, value] of tag$1 instanceof Map ? tag$1 : Object.entries(tag$1)
              .filter(([_, v]) => v != null)) {
              offset = buffer.writeUInt8(tag.getTagType(value), offset);
  ...
  var packet = createCommonjsModule(function (module, exports) {
  Object.defineProperty(exports, "__esModule", { value: true });
  ...
  class PacketReader {
      constructor(buffer, protocol = 404) {
          this.buffer = buffer;
      ...
      readVarInt() {
          const [result, offset] = varint.decodeVarInt(this.buffer, this.offset);
          return (this.offset = offset, result);
      }
      readVarLong() {
          const [result, offset] = varint.decodeVarLong(this.buffer, this.offset);
          return (this.offset = offset, result);
      }
      readPosition() {
      ...
      readNBT(options) {
          const tag = lib.decode(this.buffer.slice(this.offset), options);
  ...
  class PacketWriter {
      ...
      writeVarInt(x) {
          varint.writeVarInt(x, v => this.writeUInt8(v));
          return this;
      }
      writeVarLong(x) {
          varint.writeVarLong(x, v => this.writeUInt8(v));
          return this;
      }
      writePosition(x, y, z) {
      ...
      writeNBT(name, tag) {
          return this.write(lib.encode(name, tag));
      }
      encode() {
  ...
  class Writer extends stream.Transform {
      constructor() {
          super(...arguments);
          ...
          if (this.compressionThreshold == -1) {
              ...
              if (chunk.length < this.compressionThreshold) {
                  ...
                  return zlib.deflate(chunk, (error, buffer) => {
                      ...
                      const len = varint.encodeVarInt(chunk.length);
                      const packetLen = varint.encodeVarInt(len.length + buffer.length);
  ...
  class Connection extends events.Emitter {
      constructor(socket, isServer = false) {
          ...
          this.paused = false;
          this.packets = [];
          this.reader = new transforms.Reader;
          this.writer = new transforms.Writer;
      ...
      async resume() {
          ...
          for (const packet$1 of this.packets) {
              await Promise.resolve();
              if (!this.socket.writable)
                  break;
              this.emit("packet", new packet.PacketReader(packet$1, this.protocol));
          }
      ...
      send(packet$1) {
          const buffer = packet$1 instanceof packet.PacketWriter
              ? packet$1.encode()
              : packet$1 instanceof packet.PacketReader ? packet$1.buffer : packet$1;
          if (!this.isServer && this.state == State.Handshake) {
              const handshake = new packet.PacketReader(buffer);
          ...
          if (this.isServer && this.state == State.Login) {
              const packet$1 = new packet.PacketReader(buffer);
              if (packet$1.id == 0x2)
      ...
      setCompression(threshold) {
          if (this.isServer)
              this.send(new packet.PacketWriter(0x3).writeVarInt(threshold));
      ...
      packetReceived(buffer) {
          ...
          if (this.paused)
              ...
              this.emit("packet", new packet.PacketReader(buffer, this.protocol));
          const packet$1 = new packet.PacketReader(buffer);
          if (this.isServer) {
              if (this.state == State.Handshake) {
                  this.protocol = packet$1.readVarInt();
                  packet$1.readString(), packet$1.readUInt16();
                  this.setState(packet$1.readVarInt());
              }
              return;
          }
          if (this.state == State.Login)
              switch (packet$1.id) {
                  case 0x2:
                      ...
                      break;
                  case 0x3:
                      this.setCompression(packet$1.readVarInt());
                      break;
              }
      }
      emitError(error) {
  ...
  async function joinSession(accessToken, selectedProfile, serverId) {
      const response = await browser.default("https://sessionserver.mojang.com/session/minecraft/join", {
          method: "POST",
          headers: {
  ...
  async function hasJoinedSession(username, serverId, ip) {
      const response = await browser.default("https://sessionserver.mojang.com/session/minecraft/hasJoined?"
  ...
  class Server extends events.Emitter {
      ...
      static async encrypt(client, publicKey, privateKey, username, verify = true) {
          ...
          client.send(new packet.PacketWriter(0x1).writeString(serverId)
          ...
          const clientVerifyToken = crypto.privateDecrypt({ key: privateKey, padding: constants.RSA_PKCS1_PADDING }, encryptedVerifyToken);
          if (!verifyToken.equals(clientVerifyToken)) {
              client.end();
              throw new Error("Token verification failed");
          }
          const sharedKey = crypto.privateDecrypt({ key: privateKey, padding: constants.RSA_PKCS1_PADDING }, encryptedSharedKey);
          client.setEncryption(sharedKey);
          if (verify && !await utils.hasJoinedSession(username, serverId)) {
              client.end(new packet.PacketWriter(0x0).writeJSON({
  ...
  class ServerConnection extends lib$1.Connection {
      constructor(socket, server) {
          super(socket, true);
          this.server = server;
          this.on("changeState", state => {
              if (state == connection.State.Play && server.options.keepAlive) {
                  this.startKeepAlive();
              }
          });
      }
      startKeepAlive() {
          const ids = packet.getPacketIdMap(this.protocol);
          let id = null;
          const keepAliveInterval = setInterval(() => {
              if (id) {
                  this.end(new packet.PacketWriter(0x0).writeJSON({ text: "Timed out" }));
              }
              id = BigInt(Date.now());
              this.send(new packet.PacketWriter(ids.keepAliveC).writeUInt64(id));
  ...
  class Client extends lib$1.Connection {
      constructor(options) {
          super(new net.Socket, false);
          this.options = { ...defaultOptions, ...options };
      ...
      stateChanged(state) {
          if (state == lib$1.State.Login) {
              const disposeListener = this.onPacket(0x1, this.onEncryptionRequest.bind(this));
              this.once("changeState", disposeListener.dispose);
          }
          else if (state == lib$1.State.Play && this.options.keepAlive) {
              const ids = lib$1.getPacketIdMap(this.protocol);
              this.onPacket(ids.keepAliveC, packet => {
                  this.send(new lib$1.PacketWriter(ids.keepAliveS).writeInt64(packet.readInt64()));
              });
          }
      }
      async onEncryptionRequest(req) {
          ...
          const hashedServerId = utils.mcHexDigest(crypto.createHash("sha1")
              .update(serverId)
              .update(sharedSecret)
              .update(publicKey)
              .digest());
          if (!await utils.joinSession(this.options.accessToken, this.options.profile, hashedServerId)) {
              this.emitError(new Error("Invalid access token"));
          }
          const key = utils.mcPublicKeyToPem(publicKey);
          const encryptedSharedKey = crypto.publicEncrypt({ key, padding: constants.RSA_PKCS1_PADDING }, sharedSecret);
          const encryptedVerifyToken = crypto.publicEncrypt({ key, padding: constants.RSA_PKCS1_PADDING }, verifyToken);
          this.send(new lib$1.PacketWriter(0x1)
  ...
  var index = /*@__PURE__*/unwrapExports(lib$1);

  var index$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), index, {
    'default': index,
    __moduleExports: lib$1
  }));
  ...
  */
  ...
  var localforage$2 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), localforage$1, {
    'default': localforage$1,
    __moduleExports: localforage$1
  }));

}(crypto, stream, zlib, net, querystring, constants, dns));
